...
router.post('/expired_users' ,function(req, res, next) {

    // This route is for those Users who are not suspended and have been assigned a package but expired
    var d = new Date(); 
    var date_now = d.getTime();

    var organID = 1;
    let organ_users = connection.query(`
        SELECT * 
        FROM purchase WHERE
        price = ${organID}
    `)
    ...
    organ_users.forEach( user => {

        var expire = parseInt(user.expire);
        if( date_now > expire ){
            expired_users.push(user.user_id)
        }
    });

    res.send(expired_users)
});
...
router.post('/purcahsed_users' , function(req, res, next) {
    ...
    // This route is for those Users who are not suspended and have been assigned a package and not expired and we want to show the expire time
    ...
    var not_expired_users = [];

    organ_users.forEach( user => {

        var expire = parseInt(user.expire);
        if( date_now < expire ){
            not_expired_users.push(user.user_id)
        }
    });

    res.send(not_expired_users)
}); 
...
router.post('/add_organ_user',async function(req, res, next) {
    ...
    }else if( checkEmail[0].organ_id == 0 ){

        // If a user after personal use of their package, A company added the same user to its collection, because it has never been from a company. Organ_id can be changed to an organ ID.
        var organID = 1;

        var update_user = connection.query(`
            UPDATE users SET organ_id = ${organID}
        `);
        if ( update_user.affectedRows < 1 ) { 
            res.send({ result: false , msg: "Error in : couldn't update user"});
        }

    }else if( checkEmail[0].organ_id != 0 ){
        
        res.send({ result: false , msg: "Error: This user is registered by other organ"});
    }
